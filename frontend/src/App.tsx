...
function CategoryPage() {
  ...
  useEffect(() => {
      fetch(`${import.meta.env.VITE_API_URL}/api/achievements/all`)      
      .then((res) => res.json())
      .then((result) => {
  ...
  const handleDelete = async (deleteId: string) => {
    if (window.confirm("⚠️ Are you sure you want to DELETE this entry permanently?")) {
      try {
        fetch(`${import.meta.env.VITE_API_URL}/api/achievements/delete/${deleteId}`, { method: 'DELETE' })
        if (response.ok) {
...
function AdminPanel() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      fetch(`${import.meta.env.VITE_API_URL}/api/achievements/add`, { method: 'POST', body: submitData })
      if (response.ok) {
